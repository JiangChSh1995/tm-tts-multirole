...
> **致谢**：* 本项目是基于**类脑社区** 的 **cnfh1746_06138** 与 **kikukiku0662**的原作（酒馆TTS）进行的重构版，前端css思路，“GAL流式播放逻辑”与“多角色情感引擎”代码源自他们的脚本，已获授权修改。
---
...
* **智能注入**：`savedRefAudioBase64` 和 `promptText` 是特殊占位符。当 JSON 中包含此结构时，脚本会自动读取 UI 上传的文件和文本进行替换。
...
* **克隆/合音**：在此模式下，**不要**在 JSON 中写 `refer_wav` 或 `prompt_text` 字段。
